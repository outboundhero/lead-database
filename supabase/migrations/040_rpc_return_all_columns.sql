-- OutboundHero Database: filter + export RPCs return ALL lead columns.
--
-- Migration 039 redefined fn_filter_leads_v2 / fn_export_leads by transforming their
-- pre-existing hand-maintained SELECT projections — which never listed the new Bison /
-- validation / bounce columns. Result: the lead detail panel and CSV exports showed
-- blanks for every new field even though the data is present.
--
-- Fix: project `SELECT l.*` in both functions so every current and future column is
-- returned. The filter API and export stream pick the columns they need client-side, so
-- returning extra columns is harmless and permanently prevents blank-column regressions.
-- Bodies below captured from the live DB with only the SELECT projection swapped to l.*.
