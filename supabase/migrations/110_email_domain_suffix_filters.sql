-- 110: "Email ends with" / "Domain ends with" filters (client request 2026-09-18).
--
-- Two new keys in the filter JSON, emailSuffix and domainSuffix, each
-- {include: [], exclude: []} with no modes: lower(email) / lower(domain) must
-- END WITH one of the include values ('.in', '.org', '.co', '@gmail.com') and
-- none of the exclude values. '.co' does not match '.com'. Domain falls back to
-- the email's domain where the column is unset (Website-chip rule). Sent by
-- build-rpc-filters only when non-empty; absent keys are a no-op, so queued
-- push batches and old presets are unaffected.
--
-- fn_lead_filter_conditions is rebuilt from pg_get_functiondef of the LIVE
-- body (2026-09-18, identical to migration 092's); the only change is the two
-- branches inserted after EMAIL CONTAINS. Every wrapper (fn_filter_leads_v2,
-- fn_filter_leads_count, fn_export_leads, fn_lead_column_values,
-- fn_leads_needing_validation, push-worker) reads this function, so the
-- filters apply to the page, header dropdowns, exports and pushes alike.
...
BEGIN
  ...
  -- OR within include; each exclude is an AND-NOT. Case-insensitive. Blank
  -- values are ignored: right(x, 0) = '' would match every row. No index
  -- (same cost class as emailContains; a reverse(lower(email)) index could be
  -- added if suffix-only searches over the whole table become common).
  IF p_filters ? 'emailSuffix' THEN
    DECLARE es TEXT[] := '{}'; sv TEXT; BEGIN
      IF jsonb_array_length(COALESCE(p_filters->'emailSuffix'->'include', '[]'::jsonb)) > 0 THEN
        FOR v IN SELECT jsonb_array_elements_text(p_filters->'emailSuffix'->'include') LOOP
          sv := lower(btrim(v)); CONTINUE WHEN sv = '';
        ...
        END IF;
      END IF;
      IF jsonb_array_length(COALESCE(p_filters->'emailSuffix'->'exclude', '[]'::jsonb)) > 0 THEN
  ...
  -- DOMAIN ENDS WITH: the company domain, falling back to the email's domain
  -- where domain is unset (same rule as the Website chip), so a lead is never
  -- silently dropped for lacking a domain.
  IF p_filters ? 'domainSuffix' THEN
    DECLARE ds TEXT[] := '{}'; sv TEXT;
            dcol TEXT := 'lower(COALESCE(l.domain, split_part(lower(l.email), ''@'', 2)))'; BEGIN
      IF jsonb_array_length(COALESCE(p_filters->'domainSuffix'->'include', '[]'::jsonb)) > 0 THEN
        ...
        END IF;
      END IF;
      IF jsonb_array_length(COALESCE(p_filters->'domainSuffix'->'exclude', '[]'::jsonb)) > 0 THEN
