-- Bump COUNT(*) fallback timeout in fn_filter_leads_v2 from 15s to 60s.
--
-- When the user filters by something with many matches (e.g. job title "ceo"
-- which expands to ~915K leads), the COUNT(*) for total_count couldn't finish
-- in 15s, so the function fell back to `offset + row_count + 10000` — making
-- the UI show "10,000" results instead of the real ~915K.
--
-- Trade-off: page loads can take up to ~60s when filters are heavy. Worth it
-- for accurate counts.
...
BEGIN
  ...
  ELSE
    BEGIN
      -- Bumped from 15s → 60s so heavy filters (e.g. job title with many variants)
      -- can return accurate counts instead of falling back to "+10000" estimate.
