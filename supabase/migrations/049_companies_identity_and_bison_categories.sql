-- 049_companies_identity_and_bison_categories.sql
-- Client requirements (July 2026):
--  * Companies table becomes real: unique identifier = company name + city +
--    state (normalized into a generated company_key), auto-populated from
--    leads via fn_sync_companies(). Expected to stay under ~50k rows.
--  * Bison is the PRIMARY category source: leads arrive with category /
--    subcategory / additional category personalization variables which the
--    import ingests (category_source='bison').
--  * The categorize worker becomes a per-COMPANY fallback with caching: a
--    company name that has been categorized once (by Bison or by the worker)
--    is never re-processed; its category propagates to all its leads.

-- ── Companies: identity + category cache ───────────────────────────────────
-- The legacy Renaissance schema had UNIQUE(domain) — wrong for the new model:
-- many distinct businesses share a domain (gmail.com etc.). Identity is
-- name+city+state via company_key below.
...
ALTER TABLE companies ADD COLUMN IF NOT EXISTS subcategory TEXT;
ALTER TABLE companies ADD COLUMN IF NOT EXISTS additional_category TEXT;
ALTER TABLE companies ADD COLUMN IF NOT EXISTS category_source TEXT
  CHECK (category_source IN ('keyword', 'ai', 'manual', 'bison'));
ALTER TABLE companies ADD COLUMN IF NOT EXISTS categorized_at TIMESTAMPTZ;
ALTER TABLE companies ADD COLUMN IF NOT EXISTS company_key TEXT
  GENERATED ALWAYS AS (
    lower(TRIM(name)) || '|' || lower(TRIM(COALESCE(city, ''))) || '|' || upper(TRIM(COALESCE(state, '')))
  ) STORED;
CREATE UNIQUE INDEX IF NOT EXISTS uq_companies_key ON companies (company_key);
-- Normalized company lookup from leads (worker sample-question subquery +
-- fn_sync_companies propagation join).
CREATE INDEX IF NOT EXISTS idx_leads_company_norm ON leads (lower(TRIM(company)));

-- ── Leads: Bison category fields ───────────────────────────────────────────
ALTER TABLE leads ADD COLUMN IF NOT EXISTS subcategory TEXT;
ALTER TABLE leads ADD COLUMN IF NOT EXISTS additional_category TEXT;
...
-- ── fn_sync_companies: populate + seed + propagate ─────────────────────────
-- 1. Insert companies missing from the table (one row per name+city+state).
-- 2. Seed company categories from categorized leads (Bison or worker output).
-- 3. Propagate company categories to leads that lack one (the cache hit path:
--    an already-categorized company never re-processes).
CREATE OR REPLACE FUNCTION fn_sync_companies()
RETURNS TABLE (companies_inserted INT, companies_seeded INT, leads_propagated INT)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ── RPC updates: subcategory filter mirrors category ───────────────────────
