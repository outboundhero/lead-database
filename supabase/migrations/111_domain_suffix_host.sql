-- 111: "Domain Ends With" judges the HOST; suffix values hardened (review of 110).
--
-- The 2026-09-18 verification of 110 found that leads.domain holds URLs for
-- ~3.3% of rows (scheme, path, query, trailing slash — every one written by
-- the Bison sync, which inserts cv_domain without normalising; the CSV import
-- normalises). A raw right(domain, N) test missed those on include and, being
-- the exact negation, LET THEM THROUGH on exclude — and an exclude rides
-- push_batches.filters into the push-worker. The domain expression now reduces
-- the value to its host first. "Email Ends With" was never affected.
-- Also: NULL / blank / tab- or NBSP-padded suffix values are skipped, and a
-- non-array include/exclude is ignored instead of raising 22023.
--
-- Rebuilt from the live body (= migration 110's); only the two suffix branches
-- change. Follow-up not done here: normalise cv_domain on the Bison write path
-- and clean the existing ~290k URL-shaped domains (write-amplified; batch it).
