-- 114: "select all, minus the ones I unchecked".
--
-- The Leads page had two disconnected selection modes: `rowSelection` (the
-- CURRENT page's checked rows) and a `selectAllFiltered` flag. Changing page
-- left the flag set while the new page's rows were in neither — so the toolbar
-- said "All 340 selected" while every checkbox rendered empty and Export
-- reported "100 selected" (the first page). There was also no way to drop a
-- few rows from a select-all.
--
-- Selection is now "everything matching the filters, minus these ids". The ids
-- ride along with the ACTION (export, delete, count, push) in the filter JSON,
-- never in a saved preset or shared link, and are honoured here — in the one
-- function every consumer already calls — so the UI and the server cannot
-- disagree about what was selected.
--
-- Rebuilt from pg_get_functiondef of the LIVE body (2026-09-22); the only
-- change is the branch added before TAGS.
...
BEGIN
  ...
  -- EMAIL ENDS WITH ('.in', '.org', '@gmail.com' — client request 2026-09-18).
  -- OR within include; each exclude is an AND-NOT. Case-insensitive. NULL /
  -- blank / whitespace-only values are ignored (right(x, 0) = '' would match
  -- every row) and a non-array value is ignored rather than raising. No index
  -- (same cost class as emailContains).
  IF p_filters ? 'emailSuffix' THEN
    DECLARE es TEXT[] := '{}'; sv TEXT; BEGIN
      IF jsonb_typeof(p_filters->'emailSuffix'->'include') = 'array' THEN
        FOR v IN SELECT jsonb_array_elements_text(p_filters->'emailSuffix'->'include') LOOP
          sv := lower(btrim(v, E' \t\n\r' || chr(160))); CONTINUE WHEN sv IS NULL OR sv = '';
          es := array_append(es, format('right(lower(l.email), %s) = %L', length(sv), sv));
        END LOOP;
        IF array_length(es, 1) > 0 THEN
          conditions := array_append(conditions, '(' || array_to_string(es, ' OR ') || ')');
        END IF;
      END IF;
      IF jsonb_typeof(p_filters->'emailSuffix'->'exclude') = 'array' THEN
        FOR v IN SELECT jsonb_array_elements_text(p_filters->'emailSuffix'->'exclude') LOOP
          sv := lower(btrim(v, E' \t\n\r' || chr(160))); CONTINUE WHEN sv IS NULL OR sv = '';
          conditions := array_append(conditions, format('right(lower(l.email), %s) <> %L', length(sv), sv));
        END LOOP;
      END IF;
    END;
  END IF;

  -- DOMAIN ENDS WITH: judged on the HOST of the company domain. leads.domain is
  -- not clean — ~3.3% of rows hold a URL ('https://x.com/', 'x.com/contact-us',
  -- 'x.in/usa/…?srsltid=…'; all written by the Bison sync, which does not
  -- normalise) — and a raw suffix test would miss those on include and LEAK
  -- them on exclude (111, review finding). So: scheme stripped, cut at the first
  -- '/', '?', '#', ':' or whitespace, trailing dot dropped. A NULL/empty domain
  -- falls back to the email's domain (Website-chip rule), so a lead is never
  -- silently dropped for lacking one.
  IF p_filters ? 'domainSuffix' THEN
    DECLARE ds TEXT[] := '{}'; sv TEXT;
            dcol TEXT := 'rtrim(regexp_replace(regexp_replace(lower(btrim(COALESCE(NULLIF(btrim(l.domain), ''''), split_part(l.email, ''@'', 2)))), ''^[a-z][a-z0-9+.-]*://'', ''''), ''[/?#:[:space:]].*$'', ''''), ''.'')'; BEGIN
      IF jsonb_typeof(p_filters->'domainSuffix'->'include') = 'array' THEN
        FOR v IN SELECT jsonb_array_elements_text(p_filters->'domainSuffix'->'include') LOOP
          sv := lower(btrim(v, E' \t\n\r' || chr(160))); CONTINUE WHEN sv IS NULL OR sv = '';
          ds := array_append(ds, format('right(%s, %s) = %L', dcol, length(sv), sv));
        END LOOP;
        IF array_length(ds, 1) > 0 THEN
          conditions := array_append(conditions, '(' || array_to_string(ds, ' OR ') || ')');
        END IF;
      END IF;
      IF jsonb_typeof(p_filters->'domainSuffix'->'exclude') = 'array' THEN
        FOR v IN SELECT jsonb_array_elements_text(p_filters->'domainSuffix'->'exclude') LOOP
          sv := lower(btrim(v, E' \t\n\r' || chr(160))); CONTINUE WHEN sv IS NULL OR sv = '';
          conditions := array_append(conditions, format('right(%s, %s) <> %L', dcol, length(sv), sv));
        END LOOP;
      END IF;
    END;
  END IF;

  -- DESELECTED ROWS. "Select all N" then unchecking a few is "everything
  -- matching, MINUS these ids" — the ids are sent with the action (export,
  -- delete, push), never saved in a preset or a shared link. Every consumer
  -- reads this function, so one branch keeps them all honest: an export cannot
  -- ship a row the operator unchecked, and a delete cannot remove it.
  -- Guarded to real uuids so a malformed id is dropped rather than casting the
  -- whole array and failing the query. 1,000 is the cap the UI enforces.
  IF p_filters ? 'excludeIds' THEN
    DECLARE ex_ids TEXT;
    BEGIN
      SELECT array_agg(x)::text INTO ex_ids
        FROM (SELECT x FROM jsonb_array_elements_text(p_filters->'excludeIds') x
               WHERE x ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
               LIMIT 1000) s(x);
      IF ex_ids IS NOT NULL THEN
        conditions := array_append(conditions, format('NOT (l.id = ANY (%L::uuid[]))', ex_ids));
      END IF;
    END;
  END IF;
