-- Fix #1 (production issues May 2026):
-- The frontend sends a `selectUnknown` flag for each multi-select filter when the user
-- picks "Unknown / Empty" as a value. Both fn_filter_leads_v2 and fn_export_leads
-- were ignoring this flag, so:
--   • Picking ONLY "Unknown/Empty" returned everything (no filter was added).
--   • Picking "Unknown/Empty" + other values returned only the other values (nulls dropped).
-- Fix: respect selectUnknown in the INCLUDE branches of every multi-select filter.
...
BEGIN
  ...
  -- EXCLUDE UNKNOWN / EMPTY (same as filter RPC)
