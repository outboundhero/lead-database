-- OutboundHero Database: extend fn_export_leads with email_type, keyword exclude, and ALWAYS-ON validation gate
--
-- Differences from fn_filter_leads_v2 (migration 032):
--   • Validation gate is HARD: always require validation_status IN ('valid','catch_all').
--     Unvalidated/invalid rows must never leave the system via export — they should be sent
--     through the pre-export validation pass first (see src/lib/validation/validate-leads.ts).
--   • Bounce gate is HARD: always require is_bounced = false. The admin "include bounced"
--     toggle is filter-view only; bounced contacts are never exported.
--   • Same email_type and keyword.include/exclude logic as migration 032.
--
-- Based on migration 026 (fast PK-order default) — new conditions appended near the bottom
-- of the conditions-building block.
...
BEGIN
  ...
  -- KEYWORD (new shape: { include: [], exclude: [] }) — searches company_name,
