-- 088: whole-dataset sorting + spreadsheet-style column filters.
--
-- 1. fn_lead_filter_conditions gains a 'columnFilters' key: the KEEP-list a
--    header dropdown produces. Added to the SHARED builder so the table, the
--    count, the exports and the push preview all honour it automatically.
-- 2. fn_filter_leads_v2's sort whitelist widens from 7 columns to every column
--    a header can sort by. Header clicks used to sort only the rows already
--    loaded (getSortedRowModel over one page); they now sort the entire
--    filtered set. This also fixes 'company_size' and 'annual_revenue', which
--    the sort dropdown already offered but which fell through to created_at.
-- 3. fn_lead_column_values returns the distinct values for one column within
--    the current view, frequency-ordered and scan-capped.
--
...
BEGIN
  ...
  BEGIN
    sort_col := CASE COALESCE(NULLIF(p_sort_by, ''), 'created_at')
      ...
      -- Every column a table header can sort by (2026-08-25). Sorting used to
      -- be client-side over the loaded page only; the headers now drive this
      -- whitelist instead, so the order is over the WHOLE filtered set.
      -- 'company_size' and 'annual_revenue' were already offered by the sort
      -- dropdown but were NOT listed here, so they silently sorted by
      -- created_at.
      WHEN 'source' THEN 'source'
      WHEN 'title' THEN 'title'
      WHEN 'city' THEN 'city'
      WHEN 'state' THEN 'state'
      WHEN 'email_type' THEN 'email_type'
      WHEN 'esp' THEN 'esp'
      WHEN 'category' THEN 'category'
      WHEN 'subcategory' THEN 'subcategory'
      WHEN 'validation_status' THEN 'validation_status'
      WHEN 'replies' THEN 'replies'
      WHEN 'country' THEN 'country'
      WHEN 'seniority' THEN 'seniority'
      WHEN 'specific_industry' THEN 'specific_industry'
      WHEN 'company_size' THEN 'company_size'
      WHEN 'location_status' THEN 'location_status'
      WHEN 'additional_category' THEN 'additional_category'
      WHEN 'postal_code' THEN 'postal_code'
      WHEN 'annual_revenue' THEN 'annual_revenue'
      ELSE 'created_at'
    END;
    IF sort_col = 'id' THEN
...
-- Distinct values for one column WITHIN the current filtered view, so a header
-- dropdown offers exactly what is reachable rather than every value in the
-- table. Ordered by frequency, because on a high-cardinality column (company
-- has ~521k distinct values) the useful ones are the common ones.
--
-- SCAN CAP: an unfiltered view is 8.2M rows and this database is disk-IO bound,
-- so the scan is bounded and the result says so ("sampled": true) rather than
-- quietly costing a full table scan every time someone opens a dropdown.
CREATE OR REPLACE FUNCTION public.fn_lead_column_values(
  p_filters jsonb,
  p_column  text,
  p_search  text DEFAULT ''::text,
  p_limit   int  DEFAULT 200,
  p_scan_cap int DEFAULT 300000
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  cap          INT := LEAST(GREATEST(COALESCE(p_scan_cap, 300000), 1000), 2000000);
BEGIN
  IF p_column NOT IN (
    'source','title','company','city','state','email_type','esp','category','subcategory',
    'validation_status','replies','country','seniority','general_industry',
    'specific_industry','company_size','location_status','additional_category',
    'postal_code','annual_revenue'
  ) THEN
    RAISE EXCEPTION 'column % is not filterable', p_column;
  END IF;

  -- Same shared builder the table itself uses, so the offered values and the
  -- rows on screen can never disagree. The CALLER strips this column's own
  -- entry from p_filters — otherwise unticking a value would remove it from
  -- its own dropdown and there would be no way to tick it back on.
  conditions := fn_lead_filter_conditions(p_filters);
  IF array_length(conditions, 1) > 0 THEN
    where_clause := ' WHERE ' || array_to_string(conditions, ' AND ');
  END IF;
  ...
  IF COALESCE(btrim(p_search), '') <> '' THEN
    where_clause := where_clause
                 || CASE WHEN where_clause = '' THEN ' WHERE ' ELSE ' AND ' END
                 || format('l.%I::text ILIKE %L', p_column, '%' || btrim(p_search) || '%');
  END IF;

  sql_text := format($q$
    WITH scanned AS (
      SELECT COALESCE(NULLIF(TRIM(l.%I::text), ''), '__BLANK__') AS value
        FROM leads l %s
       LIMIT %s
    ), grouped AS (
      SELECT value, COUNT(*) AS n FROM scanned GROUP BY value
    )
    SELECT jsonb_build_object(
      'values', COALESCE((
        SELECT jsonb_agg(jsonb_build_object('value', value, 'count', n))
          FROM (SELECT value, n FROM grouped ORDER BY n DESC, value ASC LIMIT %s) t
      ), '[]'::jsonb),
      'distinct_total', (SELECT COUNT(*) FROM grouped),
      'scanned',        (SELECT COUNT(*) FROM scanned),
      'sampled',        (SELECT COUNT(*) FROM scanned) >= %s
    )
  $q$, p_column, where_clause, cap, lim, cap);
...
REVOKE ALL ON FUNCTION public.fn_lead_column_values(jsonb, text, text, int, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_lead_column_values(jsonb, text, text, int, int) TO service_role;
