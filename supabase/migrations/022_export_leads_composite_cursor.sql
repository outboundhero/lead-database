-- Fix #5 (production issues May 2026):
-- fn_export_leads paginated with `WHERE id > cursor ORDER BY id` where id is
-- a random UUID. UUID lexicographic order has nothing to do with insertion
-- order, so cursor pagination returned inconsistent slices. Range exports
-- (e.g. rows 850K–950K out of 12M) produced random/duplicate/missing rows.
--
-- Switch to ordering by (created_at, id) — stable, deterministic, and aligns
-- with how users perceive the data (newest/oldest first).
--
-- Cursor format change:
--   Before: "<uuid>"
--   After:  "<iso_timestamp>|<uuid>"
--
-- Backwards-compatible: a cursor without a '|' is treated as legacy UUID-only
-- and uses the old ordering. New code emits the composite format.
...
BEGIN
  -- Cursor: parse "<timestamp>|<uuid>" (composite) or "<uuid>" (legacy)
  ...
  -- INCLUDES (with selectUnknown handling)
  ...
  -- Composite ordering by (created_at, id) so cursor pagination is stable.
  -- Legacy callers (cursor without '|') get the old id-only ordering.
  sql_text := format(
    ...
    where_clause,
    CASE WHEN use_composite OR p_cursor IS NULL THEN 'l.created_at ASC, l.id ASC' ELSE 'l.id' END,
    p_limit, p_skip
  );
