-- 044_export_allow_unvalidated.sql
-- Fixes empty/failed exports caused by the hard validation gate in
-- fn_export_leads. Every lead currently has validation_status = NULL (no
-- Reoon/FindEmail keys yet -> the pre-export validation pass no-ops), so the
-- gate `validation_status IN ('valid','catch_all')` excluded ALL rows and every
-- export came out empty.
--
-- New gate tolerates NULL (unvalidated) rows. When validation IS configured, the
-- pre-export pass converts NULL -> valid/catch_all/invalid before export, so the
-- deliverability guarantee (only valid/catch_all, never bounced) is preserved.
-- Known-invalid and bounced rows are always excluded.
