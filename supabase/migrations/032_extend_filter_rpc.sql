-- OutboundHero Database: extend fn_filter_leads_v2 with email_type, keyword exclude, and bounce filter
--
-- New filter dimensions (additive — all backward-compatible with the Renaissance shape):
--   • emailType: { personal: boolean; general: boolean } — if exactly one true, filter to that;
--                if both true or absent, no filter applied.
--   • keyword.include / keyword.exclude — keyword is now an object { include: [], exclude: [] }
--                instead of a plain string. Searches across company_name, general_industry,
--                specific_industry, and company_overview (extended from the previous keyword field set).
--                Each include term ANDs together but uses OR across the four columns.
--   • includeBounced (top-level boolean) — when false/absent, silently filter to is_bounced=false.
--                Admin UI sets true to see bounced rows in the table view (never applied at export).
--
-- This redefines fn_filter_leads_v2 in full based on migration 024 (approximate count) — the
-- new conditions are appended near the bottom of the conditions-building block.
...
BEGIN
  ...
  -- Searches company_name, general_industry, specific_industry, company_overview.
